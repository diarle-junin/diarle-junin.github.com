...
async function gettingAllImgNames() {
    await fetch(pref_env + "institucional.json", {
        ...
        .then((data) => {
            ...
            for (var i = 0; i < dataObj.institucionales.length; i++ ) {
                processImgs(dataObj.institucionales[i], dataObj.nombres[i]);
            }
        })
        .catch((error) => {
            console.log(error);
